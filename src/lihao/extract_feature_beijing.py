...
from config import BJ_FILL_NA
pd.options.mode.chained_assignment = None


class FeatureCreate(object):
    def __init__(self):
        self.path = "data_beijing/"
        self.predict_period = 24 * 1
        self.nearest_top_num = 5
        self.label_N = 1
        self.aq_item = ['PM25', 'PM10', 'NO2', 'CO', 'O3', 'SO2']
        self.meo_item = ['temperature', 'pressure', 'humidity', 'wind_direction', 'wind_speed']
        self.time_list = []
        self.station_meo_id = ['huairou', 'mentougou', 'shunyi', 'fangshan', 'tongzhou',
                               'pinggu', 'miyun', 'daxing', 'pingchang']

    def basic_feature_aq(self, df):
        df_time = pd.DataFrame(self.time_list, columns=['utc_time'])
        feat_list = []
        feat_list_small = []
        df['stationId'] = df['stationId'].apply(lambda x: str(x).split('_')[0])
        for station_id, df_s in df.groupby('stationId'):
            ...
            co_list = df_s['CO'].tolist()
            o3_list = df_s['O3'].tolist()
            so2_list = df_s['SO2'].tolist()
            sample_id = 1
            i = 0
            while (i + self.predict_period) <= df_s.shape[0]:
                ...
                feats_dict = dict()
                feats_dict['station_id'] = station_id
                ...
                for j in range(self.predict_period):
                    ...
                    feats_dict['CO_{}hours_ago'.format(self.predict_period - j)] = co_list[i + j]
                    feats_dict['O3_{}hours_ago'.format(self.predict_period - j)] = o3_list[i + j]
                    feats_dict['SO2_{}hours_ago'.format(self.predict_period - j)] = so2_list[i + j]
                feat_list.append(feats_dict)
                if feats_dict['station_id'] in self.station_meo_id:
                    feat_list_small.append(feats_dict)
                i += 1
                sample_id += 1
        feat_df = pd.DataFrame(feat_list)
        feat_df = feat_df.reset_index(drop=True)
        print "basic_feature_aq: {}".format(feat_df.shape)
        feat_df_s = pd.DataFrame(feat_list_small)
        feat_df_s = feat_df_s.reset_index(drop=True)
        print "basic_feature_aq for small model: {}".format(feat_df_s.shape)
        return feat_df, feat_df_s
    ...
    def distance_feature_nearest(self, df_grid_all, feats_aq):
        distance_dict = cPickle.load(open("data_beijing/utils_data/all_distance.pkl", "r"))
        direction_dict = cPickle.load(open("data_beijing/utils_data/all_direction.pkl", "r"))
        process_cnt = 0
        distance_df_list = []
        for station_id, df_station in feats_aq.groupby('station_id'):
            ...
            chose_grid_cnt = 0
            for k, v in d_sort:
                if k.startswith('beijing_grid') and chose_grid_cnt < self.nearest_top_num:
            ...
            process_cnt += 1
            # print "Processed: {}, {}, Processed Cnt {}".format(station_id, distance_feats_df.shape, process_cnt)
    ...
    def design_feature_station_meo(self, df_meo):
        # 过去N个小时的 meo 均值,方差,最大值,最小值,总和,增长率,增长率的变化率,增长率的均值,增长率的方差,残差
        feats_list = []
        for index in range(df_meo.shape[0]):
            design_feat = dict()
            row = df_meo.loc[index, :].to_dict()
            design_feat['sample_id'] = row['sample_id']
            for meo in self.meo_item:
                meo_data_list = []
                for hour in range(self.predict_period):
                    key_name = "{}_{}hours_ago".format(meo, hour + 1)
                    meo_data_list.append(row[key_name])

                # 计算aq_data_list的基本统计特征
                for n in range(2, self.predict_period + 1):
                    meo_np = np.array(meo_data_list[:n])
                    new_key_pre = "{}_last_{}hours_".format(meo, n)
                    design_feat[new_key_pre + "sum"] = meo_np.sum()
                    design_feat[new_key_pre + "max"] = meo_np.max()
                    design_feat[new_key_pre + "min"] = meo_np.min()
                    design_feat[new_key_pre + "avg"] = meo_np.mean()
                ...
                for n in range(len(meo_data_list) - 1):
                    ...
                    meo_rate_list.append(increase_rate)
                    meo_res_list.append(increase_res)
                    new_key_rate = "{}_last_{}hours_increase_rate".format(meo, n)
                    new_key_res = "{}_last_{}hours_residual".format(meo, n)
                    design_feat[new_key_rate] = increase_rate
                    design_feat[new_key_res] = increase_res
                ...
                for n in range(2, len(meo_rate_list) + 1):
                    meo_rate_np = np.array(meo_rate_list[:n])
                    new_key_pre = "{}_last_{}hours_increase_rate_".format(meo, n)
                    design_feat[new_key_pre + "sum"] = meo_rate_np.sum()
                    design_feat[new_key_pre + "max"] = meo_rate_np.max()
                    design_feat[new_key_pre + "min"] = meo_rate_np.min()
                    design_feat[new_key_pre + "avg"] = meo_rate_np.mean()
                ...
                for n in range(2, len(meo_res_list) + 1):
                    meo_res_np = np.array(meo_res_list[:n])
                    new_key_pre = "{}_last_{}hours_residual_".format(meo, n)
                    design_feat[new_key_pre + "sum"] = meo_res_np.sum()
                    design_feat[new_key_pre + "max"] = meo_res_np.max()
                    design_feat[new_key_pre + "min"] = meo_res_np.min()
                    design_feat[new_key_pre + "avg"] = meo_res_np.mean()
        ...
        feats_df = pd.DataFrame(feats_list)
        feats_df = feats_df.reset_index(drop=True)
        print "design_feature_station_meo: {}".format(feats_df.shape)
        return feats_df
    ...
    def master(self, time_range, submit_date):
        ...
        if BJ_FILL_NA:
            try:
                df_aq = pd.read_table(self.path + "use_data/{}_aq_filled.csv".format(submit_date), sep=",")
            except:
                df_aq = pd.read_table(self.path + "use_data/{}_aq.csv".format(submit_date), sep=",")
        else:
            df_aq = pd.read_table(self.path + "use_data/{}_aq.csv".format(submit_date), sep=",")
        df_grid_meo = pd.read_table(self.path + "use_data/{}_grid_meo.csv".format(submit_date), sep=",")
        aq_feat_df, aq_feat_df_small = self.basic_feature_aq(df_aq)
        grid_meo_feat_df = self.basic_feature_grid_meo(df_grid_meo)
        nearest_grid_meo_feat_df = self.distance_feature_nearest(grid_meo_feat_df, aq_feat_df)
        aq_design_feat_df = self.design_feature_aq(aq_feat_df)
